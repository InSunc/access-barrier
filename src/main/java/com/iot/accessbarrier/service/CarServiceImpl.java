...
//        validatePlateNumber(car.getPlateNumber());
        final Car[] result = new Car[1];
        System.out.println(">>>>>>>> " + car.getId());
        carRepository.findById(car.getId()).ifPresentOrElse(
                (e) -> {
                    e.setOwner(car.getOwner());
                    e.setBrand(car.getBrand());
                    e.setPlateNumber(car.getPlateNumber());
                    result[0] = carRepository.save(e);
                },
                () -> {
                    result[0] = carRepository.save(car);
                }
        );
